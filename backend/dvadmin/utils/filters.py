...
# 添加缺失的常量定义
try:
    from django_filters.constants import FILTER_FOR_DBFIELD_DEFAULTS
except ImportError:
    # 如果无法导入，则定义默认值
    from django_filters import filters
    FILTER_FOR_DBFIELD_DEFAULTS = {
        models.AutoField: {'filter_class': filters.NumberFilter},
        models.BigIntegerField: {'filter_class': filters.NumberFilter},
        models.BooleanField: {'filter_class': filters.BooleanFilter},
        models.CharField: {'filter_class': filters.CharFilter},
        models.CommaSeparatedIntegerField: {'filter_class': filters.CharFilter},
        models.DateField: {'filter_class': filters.DateFilter},
        models.DateTimeField: {'filter_class': filters.DateTimeFilter},
        models.DecimalField: {'filter_class': filters.NumericRangeFilter},
        models.EmailField: {'filter_class': filters.CharFilter},
        models.Field: {'filter_class': filters.CharFilter},
        models.FilePathField: {'filter_class': filters.CharFilter},
        models.FloatField: {'filter_class': filters.NumericRangeFilter},
        models.IntegerField: {'filter_class': filters.NumberFilter},
        models.GenericIPAddressField: {'filter_class': filters.CharFilter},
        models.NullBooleanField: {'filter_class': filters.BooleanFilter},
        models.PositiveIntegerField: {'filter_class': filters.NumberFilter},
        models.PositiveSmallIntegerField: {'filter_class': filters.NumberFilter},
        models.SlugField: {'filter_class': filters.CharFilter},
        ...
        models.DurationField: {'filter_class': filters.NumberFilter},
        models.UUIDField: {'filter_class': filters.UUIDFilter},
        models.URLField: {'filter_class': filters.CharFilter},
    }
...
class CustomDjangoFilterBackend:
    """
    自定义过滤器，支持更多字段类型
    """
    lookup_prefixes = {
    ...
    filterset_base = FilterSet  # 添加缺失的属性
    raise_exception = True  # 添加缺失的属性
    ...
    def get_filterset_class(self, view, queryset=None):
        ...
        if filterset_fields and queryset is not None:
            ...
            class AutoFilterSet(self.filterset_base):
                class Meta:
                    model = queryset.model if queryset else view.get_queryset().model
                    fields = "__all__"
                    # 添加对文件字段的覆盖，防止错误
                    filter_overrides = {
                        models.FileField: {
                        ...
                        **FILTER_FOR_DBFIELD_DEFAULTS  # 保留默认覆盖
                    }
    ...
    def filter_queryset(self, request, queryset, view):
        ...
        if filterset.__class__.__name__ == "AutoFilterSet":
            ...
            orm_lookups = [
                self.construct_search(field_name, lookup_expr) for field_name, lookup_expr in orm_lookup_dict.items()
            ]
            # print(orm_lookups)
            conditions = []
            queries = []
